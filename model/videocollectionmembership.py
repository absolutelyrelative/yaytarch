...
def getvideocollectionmembershipbyid(id, type="VIDEO"):
    ...
    match type:
        case "VIDEO":
            try:
                result = db.execute('SELECT * FROM videocollectionmembership WHERE videocollectionmembership.videoid = ' + str(id)
                ).fetchall()
        ...
        case "COLLECTION":
            try:
                result = db.execute('SELECT * FROM videocollectionmembership WHERE videocollectionmembership.collectionid = ' + str(id)
                ).fetchall()
            except db.Error as db_error:
                print(bcolors.WARNING + "Database error:" + bcolors.ENDC)
                print("{}".format(db_error))
